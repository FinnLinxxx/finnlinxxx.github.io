...
$ rosservice call /scan_ms50 1 2
Auf dem Tracker-Laptop (remote) TScanClient.exe starten -> ConnectScanner -> 5*UpdateHandler-> Socket - (StartPolyline)
$ rosrun igros_emscon igros_emscon_tscan >> 0gon.txt
Dann (Tracker-Laptop (remote) TScanClient.exe) -> StartScanner -> StartScannerMeasurement
$ roslaunch igros_ur move_juri.launch joints_file:=/home/finn/workspace_ur/src/igros_ur/trajectories/$(rosparam get UR_traj_file_name) speed_factor:=0.3
anschließend (Tracker-Laptop (remote) TScanClient.exe) -> StopScannerMeasurement -> StopAcquisitionMade -> DisconnectScanner -> Close
bei igros_emscon_tscan -> Strg + C
Immer wieder so wiederholen...
```
